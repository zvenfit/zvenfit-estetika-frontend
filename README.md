...
Статический лендинг из Webflow, клиентский код на чистом JavaScript и одна облачная функция Yandex Cloud. HTTP-запрос сохраняет заявку или подписку вместе с версией явных согласий в YDB и сразу возвращает подтверждение; уведомления в Telegram асинхронно доставляет минутный timer с повторными попытками.
