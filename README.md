...
`leads` содержит только бизнес-заявки: каждая отправка формы — отдельный лид. `newsletter_subscriptions` содержит единственную подтверждённую текущую строку на нормализованный телефон, а `newsletter_consent_events` — неизменяемую историю запросов opt-in, доверенных подтверждений и отписок. Публичная форма записывает только `opt_in_requested` и всегда оставляет номер suppressed до подтверждения владения через доверенную интеграцию. Повторный публичный запрос не может снять существующую отписку.

`telegram_outbox` не является бизнес-доменом: он хранит снимок уведомления и технические состояния `pending`, `sending`, `sent`, `failed`. Запись домена и его outbox-сообщения выполняется в одной YDB-транзакции, поэтому успешный HTTP-ответ не может потерять уведомление. После terminal-состояния PII-снимок очищается, а в технической записи остаются только UUID и доставка.

Отписка идемпотентна, всегда фиксирует отдельное consent-событие и suppression-тумбстоун, даже если активной подписки ещё нет. Текущее состояние меняется только более новым событием; при одинаковом времени отзыв согласия имеет приоритет. Публичного endpoint с одним только телефоном нет: интеграция с провайдером должна использовать доверенный webhook либо подписанную одноразовую ссылку и проверять suppression перед отправкой.
...
Для ручной проверки откройте `/?utm_source=test`, отправьте запрос на подписку, затем заявку на `/form/` и убедитесь, что в логе или сообщении Telegram есть маркировка. Запрос на подписку не должен становиться `active` без доверенного подтверждения номера.
