...
Продуктовые условия запуска, открытые решения и контентный backlog перенесены
в Personal AI Workspace 2026-09-17. [Канонические заметки и карта переноса](docs/personal-ai-workspace.md#перенесённый-продуктовый-контекст).

## Ближайший порядок работ

Приоритеты и условия допуска к приёму заявок / рекламному трафику каноничны
в плане Workspace **«ZvenFit Estetika — запуск и развитие»**.
[Как найти план](docs/personal-ai-workspace.md#перенесённый-продуктовый-контекст).
Ниже остаются технические задачи и их результаты; они сами по себе не
подтверждают продуктовую или юридическую готовность запуска.
...
- **Юридическая готовность форм** — решение о способе согласия, история подтверждения
  ...
  После согласования изменения форм выполняются отдельной технической задачей.
- [x] **Production smoke-тест форм** — маркированные заявка и подписка сохранены в YDB и доставлены
  в Telegram 2026-08-17. После DNS-first failover повторная проверка заявки завершилась
  `status=sent`, `attempts=1`, `last_error=null`; проверка целей Метрики остаётся отдельной задачей
...
- [x] **Асинхронная доставка Telegram** — HTTP-запрос завершается после сохранения в YDB; timer обрабатывает до пяти записей, проверяет DNS и резервные IPv4 безопасными `HEAD`-запросами, кэширует здоровый маршрут и не повторяет неоднозначный POST в том же вызове
...
- [ ] **HTTP-заголовки безопасности на Cloud CDN** — добавить как статические заголовки ответов клиентам на CDN, а не через HTML `<meta>` ([документация Yandex Cloud CDN](https://yandex.cloud/ru/docs/cdn/operations/resources/configure-headers))
  - Порядок внедрения: сначала обратимые заголовки; HSTS — только после проверки стабильного HTTPS.
...
- **Содержание политики и согласий** — список вопросов и расхождение исходных
  формулировок ведутся в [плане Workspace](docs/personal-ai-workspace.md#перенесённый-продуктовый-контекст).
  Техническая реализация следует только итоговому согласованному требованию.
...
Канонический список продуктовых улучшений и необходимых материалов находится
в [плане Workspace](docs/personal-ai-workspace.md#перенесённый-продуктовый-контекст).
Там сохранены все восемь исходных пунктов и их незакрытое состояние на момент
переноса; здесь отдельная копия их статусов не ведётся.
...
Продуктовые ограничения и необязательные возможности перенесены в карточку
**«Сайт ZvenFit Estetika» (`PROD-002`)**.
[Как найти карточку](docs/personal-ai-workspace.md#перенесённый-продуктовый-контекст).
Инженерный паритет контролируется по [`docs/upstream-parity.md`](docs/upstream-parity.md).
Технические различия остаются здесь:
