...
| `ZVENFIT_ESTETIKA_MONIUM_METRICS_TIMEOUT_MS` | Repository-level deadline каждой стадии OTLP collect/export/force flush/shutdown; workflow передаёт его функции как `MONIUM_METRICS_TIMEOUT_MS`. По умолчанию `5000`, диапазон `100–5000`. Проектное имя защищает Estetika от одноимённой organization variable |
