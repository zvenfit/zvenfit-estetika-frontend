...
- Этот репозиторий зарегистрирован как `REP-002`, продукт — `PROD-002`
  («Сайт ZvenFit Estetika»), домен — `zvenfit`. Связи отражены в
  [`repo-manifest.json`](repo-manifest.json).
- Подключение Personal AI Workspace разрешено владельцем. Продуктовый контекст,
  цели и планы находятся в его канонических карточках; технические знания,
  исходники и проверки остаются в этом репозитории.
- Локальный указатель находится только в Git metadata:
  `git rev-parse --git-path personal-ai-workspace/local.json`. Перед чтением
  выбранных заметок Workspace применяйте его exact-file cloud preflight.
  Недоступная связь не разрешает сканировать весь vault или соседние проекты.
- [`project-checks.json`](project-checks.json) перечисляет существующие проверки.
  `python3 scripts/check.py --list` показывает команды и digest; исполнение —
  с `--execute-reviewed <digest>` и нужными флагами `--allow-writes` /
  `--allow-network` в отдельном tool/OS sandbox. Runner сам не создаёт sandbox.
- Локальный adopt не разрешает commit, push, deploy, production-изменения и
  внешние сообщения. Подробности: [`docs/personal-ai-workspace.md`](docs/personal-ai-workspace.md).
...
- Для технических знаний используйте версионируемую базу этого репозитория в
  `knowledge-base/`. Продуктовый контекст ведётся отдельно в Personal AI Workspace.
- Новые бизнес-знания и исследования направляйте в домен `zvenfit` Workspace;
  новые существенные технические решения оформляйте как ADR в `docs/decisions/`.
  [Правила выбора места и примеры](docs/personal-ai-workspace.md#как-добавлять-новые-знания).
- Перед задачами про мониторинг, инфраструктуру и принятые решения начинайте с
  `knowledge-base/_index.md` и релевантной заметки.
- Фиксируйте в базе только подтверждённые устойчивые выводы; пошаговые инструкции
  и полные селекторы остаются в профильных runbook и конфигурации.
- `knowledge-base/` можно stage, commit и push только в настроенный Git remote этого
  проекта после проверки на секреты, персональные данные и production payloads.
- Не синхронизируйте и не копируйте `knowledge-base/` в Wiki, DataCatalog, Obsidian,
  межпроектную память или отдельную поверхность базы знаний.
...
| `knowledge-base/_index.md` | Индекс подтверждённых проектных знаний и решений |
